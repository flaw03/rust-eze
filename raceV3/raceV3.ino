...
void loop() {
    ...
    if (abs(sensorErrorLeft) < 10 && abs(sensorErrorRight) < 10) {
    accelerationFactor += 0.01; // Augmentez progressivement la vitesse si le robot est bien centré
    } else {
    accelerationFactor -= 0.01; // Réduisez la vitesse si le robot s'éloigne de la ligne
    }

    accelerationFactor = constrain(accelerationFactor, minSpeed / baseSpeedLeft, maxSpeed / baseSpeedLeft);
    baseSpeedLeft *= accelerationFactor;
    baseSpeedRight *= accelerationFactor;
